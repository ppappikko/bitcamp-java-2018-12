...
//    WARN  : 잠재적인 위험을 안고 있는 오류일 경우. 예) 서버와의 소켓 연결을 close() 하지 못했을 때.
//    INFO  : 애플리케이션의 주요 실행 정보. 예) 어떤 IP의 클라이언트 접속 정보. 연결 종료.
//    DEBUG : 애플리케이션의 내부 실행 상태를 추적해 볼 수 있는 정보.
//            예) Mybatis가 실행하는 SQL, SQL을 실행할 때 in-parameter에 설정되는 값.
//    TRACE : DEBUG 보다 더 상세한 정보. 예) IoC 컨테이너가 생성한 객체 목록
//
// => 등급에 따른 출력 제어
